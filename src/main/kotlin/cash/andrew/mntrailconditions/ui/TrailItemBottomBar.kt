...
class TrailItemBottomBar(
  ...
  private fun favoriteClickListener(trail: TrailViewModel, favoriteTrailsPref: Preference<Set<String>>) : (view: View) -> Unit = { view ->
    val favoriteTrails = favoriteTrailsPref.get()
    val contains = favoriteTrails.contains(trail.name)
    trail_favorite.setImageResource(if (contains) R.drawable.ic_favorite_selected else R.drawable.ic_favorite_unselected)
    val updatedTrails = favoriteTrails.toMutableSet().apply { if (contains) remove(trail.name) else add(trail.name) }
