...
abstract class BindableAdapter<T>(val context: Context) : BaseAdapter() {
    ...
    override fun getView(position: Int, view: View?, container: ViewGroup) =
            (view ?: newView(inflater, position, container))
                .apply { bindView(getItem(position), position, this) }
    ...
    override fun getDropDownView(position: Int, view: View?, container: ViewGroup) =
            (view ?: newDropDownView(inflater, position, container))
                    .apply { bindDropDownView(getItem(position), position, this) }

    /** Create a new instance of a drop-down view for the specified position.  */
    open fun newDropDownView(inflater: LayoutInflater, position: Int, container: ViewGroup) =
